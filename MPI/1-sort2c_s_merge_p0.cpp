...
int main()
{
    ...
    if(my_rank == 0){
        ...
        if(my_rank == 1){
            ...
            A = (int *) malloc ((sizeRecvBuffer) * sizeof (int));
            MPI_Recv(A, sizeRecvBuffer, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
        }
        ...
        MergeSortSerial(A, 0, sizeRecvBuffer - 1);

        MPI_Send(A, sizeRecvBuffer, MPI_INT, 0, 0, MPI_COMM_WORLD);
    }         
